...
namespace Industry.Tiles
{
    ...
    => Main.chest[Chest.FindChest(i - 2, j - 3)];
        ...
        {
            if (!Main.LocalPlayer.WithinRange(new Vector2(i, j), 8))
                return;

            // Если структура уже валидна — ничего не рисуем
            if (CheckMultiblock(i, j, out _))
                return;

            DrawChestGhost(spriteBatch, i - 2, j - 3);
            DrawChestGhost(spriteBatch, i + 2, j - 3);
        }
        private void DrawChestGhost(SpriteBatch spriteBatch, int x, int y)
        {
            int chestIndex = Chest.FindChest(x, y);
            if (chestIndex >= 0)
                return; // сундук уже есть

            Texture2D tex = TextureAssets.ChestStack[0].Value; // обычный сундук
            Vector2 worldPos = new Vector2(x * 16, y * 16);
            Vector2 screenPos = worldPos - Main.screenPosition;

            Rectangle frame = new Rectangle(0, 0, 32, 32); // 2x2 сундук
